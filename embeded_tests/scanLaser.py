...
from i2cylib.utils import Echo
...
while True:
    ...
    ECHO.print("camera IO time: {:.2f}ms".format((time.time() - t1) * 1000))
    # blur = cv2.bilateralFilter(frame, 9, 75, 75)
    t1 = time.time()
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)  # 我们对帧的操作
    ECHO.print("color conversion time: {:.2f}ms".format((time.time() - t1) * 1000))
    t1 = time.time()
    ret, mask = cv2.threshold(gray, 210, 255, cv2.THRESH_BINARY)
    ECHO.print("binary time: {:.2f}ms".format((time.time() - t1) * 1000))
    t1 = time.time()
    ...
    # ECHO.print(cv2.contourArea(cnt))
    x, y, w, h = cv2.boundingRect(cnt)
    if 1 < w * h < 100:  # &(w*h<600):

        cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
        cv2.circle(frame, (x + w // 2, y + h // 2), 2, (0, 255, 0), 3)
        ECHO.buttom_print([x + w // 2, y + h // 2])
    ECHO.print("search time: {:.2f}ms".format((time.time() - t1) * 1000))

    ECHO.print("FPS: {:.2f}".format(1 / (time.time() - t0)))
